...
public class SplitInTwoModifier : IPlayerWeaponModifier
{
	...
	public const float SPLIT_ANGLE = 90f;
	...
	public void OnBulletDestroy(PlayerBullet playerBullet)
	{
		if (playerBullet.ModifiersProcCount[ModifierType.SplitInTwoModifier] > 0)
		{
			...
			leftBullet.Transform.localEulerAngles = new Vector3(0f, 0f, playerBullet.Transform.localEulerAngles.z - SPLIT_ANGLE / 2f);
			...
			rightBullet.Transform.localEulerAngles = new Vector3(0f, 0f, playerBullet.Transform.localEulerAngles.z + SPLIT_ANGLE / 2f);
